...
def show():
    st.header("✈️ Day 8: 最後衝刺 & 返回溫暖的家")

    # ==========================================
    # 0. 航班資訊 (置頂提醒)
    # ==========================================
    with st.container(border=True):
        st.subheader("🛫 航班資訊：國泰航空 CX531")
        st.markdown("""
        * **16:40** NGO 起飛 (中部國際機場)
        * **19:15** TPE 抵達 (台北桃園)
        * 建議 **13:40** 前抵達機場
    ...
    st.info("💡 **麥當勞**：試試看日本限定的鬆餅漢堡 (McGriddles) 或期間限定派。")
    ...
    * **發車時間**：**12:10** (預計 12:55 抵達機場)
    ...
    st.warning("⚠️ **風險提示**：若巴士客滿，請立刻啟用下方的「方案 B」。")
    ...
    # 時刻表 (摺疊區塊)
    # ==========================================
    with st.expander("🚄 點我看：金山站 -> 機場時刻表 (若巴士客滿請點此)"):
        st.markdown("[🔗 點我去官網時刻表確認](https://trainbus.meitetsu.co.jp/meitetsu-transfer-zh-tw/pc/transfer/DepArrTimeList?snode=00009406&gnode=00004372&date=2026-01-17&depTime=16)")
        ...
        st.caption("🔵 μ-SKY 最快但全車指定席 / 🔴 特急推薦搭自由座車廂即可")
